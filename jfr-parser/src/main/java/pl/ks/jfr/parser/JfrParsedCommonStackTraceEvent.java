...
import static pl.ks.jfr.parser.tuning.AdditionalLevel.SPANS;
...
public interface JfrParsedCommonStackTraceEvent extends JfrParsedEventWithTime, JfrParsedEventWithThread {
    ...
    Comparator<JfrSpanInfo> SPAN_INFO_COMPARATOR = Comparator.comparing(JfrSpanInfo::getEventTime).thenComparing(JfrSpanInfo::getTag);
    ...
    default void addCommonStackTraceElements(List<String[]> fullStackTrace, Set<AdditionalLevel> additionalLevels) {
        ...
        if (additionalLevels.contains(SPANS) && getSpans() != null) {
            String[] spans = new String[getSpans().size()];
            int i = 0;
            for (JfrSpanInfo span : getSpans()) {
                spans[i++] = span.getTag() + "_[i]";
            }
            fullStackTrace.add(spans);
        }
